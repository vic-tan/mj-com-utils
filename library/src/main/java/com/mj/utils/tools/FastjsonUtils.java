...
/*	public static String getData(JSONObject json) {
        try {
			if (StringUtils.isEmpty(json.getString(JsonConstants.JSON_DATA)) || StringUtils.isEquals(json.getString(JsonConstants.JSON_DATA), "null")) {
				return "[]";
			} else {
    ...
    /**
     * 把json 中的"class" key  替换成"class_bean" key ,这样做是为了跟 class是关键字，在java 中不能用关键字来做字段
     *
     * @param responseBody
     * @return
     */

    public static String replaceClass(String responseBody) {
        if ("".equals(responseBody) || null == responseBody || "null".equals(responseBody)) {
            return responseBody;
        }
        return responseBody.replace("\"class\":", "\"class_bean\":");
    }
    ...
    public static <T> List<T> parseToPathCourseList(String json, Class<T> clazz)
            throws JSONException {
     ...
     * @throws JSONException
     */
    public static <T> List<T> parseToObjectList(com.alibaba.fastjson.JSONObject json, Class<T> clazz,
    ...
    public static String beanToJson(Object bean) {
        ...
        for (Field f : fields) {
            try {
                ...
                Method method = cl.getDeclaredMethod("get" + fieldName.toString(), new Class[]{});// 获取类中的get方法
                json.append(objectToJson(method.invoke(bean, new Object[]{})));
